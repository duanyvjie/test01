	...
	$.ajax({
		...
		success:function(data){
			if(data.delResult == "true"){//删除成功：移除删除行
				...
				alert("删除论文【"+obj.attr("username")+"】成功");
			}else if(data.delResult == "false"){//删除失败
				//alert("对不起，删除用户【"+obj.attr("username")+"】失败");
				changeDLGContent("对不起，删除论文【"+obj.attr("username")+"】失败");
			}else if(data.delResult == "notexist"){
				//alert("对不起，用户【"+obj.attr("username")+"】不存在");
				changeDLGContent("对不起，论文【"+obj.attr("username")+"】不存在");
			}
		},
		error:function(data){
			//alert("对不起，删除失败");
...
$(function(){
	...
	$(".modifyUser").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/paper/tomodify?id="+ obj.attr("id");
	});
	...
	$(".deleteUser").on("click",function(){
		userObj = $(this);
		alert("qqq")
		changeDLGContent("你确定要删除论文【"+userObj.attr("username")+"】吗？");
		openYesOrNoDLG();
	});
